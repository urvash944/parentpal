...
import { generateAnimalCountingGame } from "../../api/gamesAPI";
...
export default function AnimalCountingGame({ onExit }) {
  const { user } = useAuth();
  ...
  async function load() {
    ...
    try {
      const d = await generateAnimalCountingGame(activeChild);
      ...
      {data && !done && rounds[round] && (
        <div>
          <div className={styles.bigDisplay}>
            <p className={styles.emojiRow}>{rounds[round].displayString}</p>
          </div>
          <QuestionCard
            key={round}
            question={`How many ${rounds[round].animalEmoji} are there?`}
