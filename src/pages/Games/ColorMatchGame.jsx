...
import { generateColorMatchGame } from "../../api/gamesAPI";
...
export default function ColorMatchGame({ onExit }) {
  const { user } = useAuth();
  ...
  async function load() {
    ...
    try {
      const d = await generateColorMatchGame(activeChild);
      ...
      {data && !done && rounds[round] && (
        <div>
          <div className={styles.bigDisplay}>
            <div className={styles.colorSwatch} style={{ background: rounds[round].colorHex }} />
          </div>
          <QuestionCard
            key={round}
            question="What color is this?"
            options={rounds[round].options}
            answer={rounds[round].colorName}
            questionNum={round + 1}
            total={rounds.length}
