...
import { generateWordBuilderGame } from "../../api/gamesAPI";
...
export default function WordBuilderGame({ onExit }) {
  const { user } = useAuth();
  ...
  async function load() {
    ...
    try {
      const d = await generateWordBuilderGame(activeChild);
      ...
      {data && !done && rounds[round] && (
        <div>
          <div className={styles.bigDisplay}>
            <span className={styles.wordEmoji}>{rounds[round].emoji}</span>
            <p className={styles.wordDisplay}>{rounds[round].displayWord}</p>
          </div>
          <QuestionCard
            key={round}
            question="Which letter completes the word?"
            options={rounds[round].options}
            answer={rounds[round].missingLetter}
            questionNum={round + 1}
            total={rounds.length}
